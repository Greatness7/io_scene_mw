...
class SceneNode:
    ...
    @cached_property
    def name(self):
        # discard numeric suffixes generated by blender (e.g., ".001", ".002")
        do_strip_suffixes = self.exporter.strip_numeric_suffixes
        # always strip from these
        if name in ("Bip01", "Root Bone", "Bounding Box"):
            do_strip_suffixes = True
        if do_strip_suffixes:
            return strip_blender_suffix(self.source.name)
...
class Mesh(SceneNode):
    ...
    def create(self):
        ...
        for i, tri in enumerate(outputs):
            ...
            tri.data.update_center_radius(exact=True)
    ...
    def create_vertex_groups(self, trishape, skin_data):
        ...
        for i, bone_data in enumerate(skin.data.bone_data):
            ...
            # apply center/radius
            bone_data.update_center_radius(trishape.data.vertices[j], exact=True)
...
class Material(SceneNode):
    ...
    def create_material_property(self, ni_object, bl_prop):
        ...
        if self.exporter.strip_numeric_suffixes:
            ni_prop.name = strip_blender_suffix(bl_prop.material.name)
        else:
