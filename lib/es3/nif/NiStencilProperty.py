...
class NiStencilProperty(NiProperty):
    ...
    stencil_function: uint32 = TestFunction.TEST_NEVER
    stencil_ref: uint32 = 0
    stencil_mask: uint32 = 0xFFFFFFFF
    fail_action: uint32 = Action.ACTION_KEEP
    pass_z_fail_action: uint32 = Action.ACTION_KEEP
    pass_action: uint32 = Action.ACTION_KEEP
    draw_mode: uint32 = DrawMode.DRAW_CCW_OR_BOTH
    ...
    def load(self, stream):
        ...
        self.stencil_function = TestFunction(stream.read_uint())
        self.stencil_ref = stream.read_uint()
        self.stencil_mask = stream.read_uint()
        self.fail_action = Action(stream.read_uint())
        self.pass_z_fail_action = Action(stream.read_uint())
        self.pass_action = Action(stream.read_uint())
        self.draw_mode = DrawMode(stream.read_uint())
    ...
    def save(self, stream):
        ...
        stream.write_uint(self.pass_z_fail_action)
